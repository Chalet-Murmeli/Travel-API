...
with st.sidebar:
    ...
    ⚠️ Bitte Google Maps API Key eingeben! Ohne Key können Routen und PDF nicht erstellt werden.
    </div>
...
def get_sbb_ticket_price(start_loc, dest_loc, date_obj, time_obj):
    try:
        r = requests.get("https://timetable.search.ch/api/route.json",
                         params={"from": start_loc, "to": dest_loc,
...
def generate_pdf_final(route_auto, route_transit, start, destination, date, time_input, output_file="Reisevergleich.pdf"):
    ...
    try:
        if route_auto:
            ...
            if auto_img_file: pdf.image(auto_img_file, x=10, y=pdf.get_y(), w=90)
        if route_transit:
            ov_img_file = static_map_save(route_transit, start, destination, "tmp_ov.png")
            if ov_img_file: pdf.image(ov_img_file, x=105, y=pdf.get_y(), w=90)
        pdf.ln(80)
    except Exception as e:
